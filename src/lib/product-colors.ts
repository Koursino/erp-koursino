...
// reliably. Same approach as /stock/products.
...
/**
 * product_id → "Noir", for the given articles (or all of them when `productIds`
 * is omitted). Never throws: a missing attribute yields an empty map, so every
 * call site degrades to "—".
 */
export async function loadColorLabels(
  ...
  const labelByValueId = new Map(
    ((labelsRes.data ?? []) as { id: string; label: string }[]).map((v) => [v.id, v.label])
  );
  ...
  for (const row of (valuesRes.data ?? []) as { product_id: string; value_id: string }[]) {
    ...
    const label = labelByValueId.get(row.value_id);
    if (label) colorById.set(row.product_id, label);
  }
